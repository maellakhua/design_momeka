...
angular.module('starter.controllers', [])
        ...
        .controller('siteUrlCtrl', function ($scope, Url, $http) {
            ...
            $scope.assignUrl = function (url) {
                ...
                $scope.tempurl=Url.all().substring(Url.all().length-1, Url.all().length);
                if($scope.tempurl.toString()=="/"){
                    Url.setUrl(Url.all().substring(0, Url.all().length-1));
                }
                console.log($scope.tempurl.toString());
                $http({
